...
class PurchaseMigrationWizard(models.TransientModel):
    ...
    def action_fetch_po(self):
        ...
        # fetching res_partner from odoo 17 database and creating it in odoo 18 database
        ...
        # fetching product_template from odoo 17 database and creating it in odoo 18 database
        if self.is_product:
            product_domain = [('type', '!=', 'service')] if self.is_goods else [('type', '=', 'service')]
            if (self.is_goods and self.is_service) or (not self.is_goods and not self.is_service):
                product_domain = []
            db_1_products = models_1.execute_kw(db_1, uid_db1, password_db_1, 'product.template', 'search_read', [],
                                                {'domain': product_domain,
                                                 'fields': ['name', 'type', 'list_price', 'default_code',
                                                            'purchase_ok', 'standard_price']})
        ...
        # fetching purchase_order from odoo 17 database and creating it in odoo 18 database
        if self.is_po:
            po_state = []
            if self.is_rfq:
                po_state.append('draft')
            if self.is_rfq_sent:
                po_state.append('sent')
            if self.is_to_approve:
                po_state.append('to_approve')
            if self.is_purchase:
                po_state.append('purchase')
            if self.is_done:
                po_state.append('done')
            if self.is_cancelled:
                po_state.append('cancel')
            po_domain = [('state', 'in', po_state)] if po_state else po_state
            # fetching po wrt domain
            ...
            for rec in db_1_po:
                ...
                if partner and not db_2_po:
                    ...
                    db1_order_line = models_1.execute_kw(db_1, uid_db1, password_db_1, 'purchase.order.line',
                                                         'search_read',
